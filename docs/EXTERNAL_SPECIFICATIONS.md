...
| `dates` | date[] | ✓ | 予定日。複数指定時は日付ごとに独立した予定を作成 |
| `date` | date | - | 単一日作成との後方互換および編集時の予定日 |
...
- `dates` は重複を除去して処理され、すべての予定が1トランザクションで作成されます

### 詳細 (GET /plans/:id)
予定の詳細情報と参加者リストを表示。

### 編集 (PUT/PATCH /plans/:id)
単一予定の日付とその他のフィールドを更新可能。`last_edited_by` が自動記録されます。

### 削除 (DELETE /plans/:id)
予定を個別に削除。関連する `plan_participants` も削除されます。
