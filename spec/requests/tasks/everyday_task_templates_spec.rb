...
RSpec.describe 'EverydayTaskTemplates', type: :request do
  ...
  describe 'PATCH /update' do
    it 'updates the template' do
      patch everyday_task_template_path(everyday_task_template),
            params: { everyday_task_template: { name: 'Updated Name' } }
  ...
  describe 'POST /bulk_add' do
    ...
    it 'creates tasks from templates' do
      expect do
        post bulk_add_everyday_task_template_path(everyday_task_template), params: { date: Time.zone.today.to_s }
      end.to change(Task, :count).by(2)
    end

    it 'redirects back' do
      post bulk_add_everyday_task_template_path(everyday_task_template), params: { date: Time.zone.today.to_s }
      expect(response).to redirect_to(calendar_path(date: Time.zone.today.to_s))
    end
  end
end
