class PlanParticipantsController < ApplicationController
  ...
  def update
    @plan_participant = PlanParticipant.joins(user: :family).where(families: { id: current_user.family_id }).find_by(id: params[:id])

    if @plan_participant && @plan_participant.update(status: params[:status])
      ...
      respond_to do |format|
        format.turbo_stream do
          ...
          render turbo_stream: turbo_stream.update("daily_details", partial: "calendar/daily_view", locals: { date: @date, scope: scope })
        end
