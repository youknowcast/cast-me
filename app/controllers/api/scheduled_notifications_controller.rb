...
module Api
  class ScheduledNotificationsController < ApplicationController
    ...
    def verify_api_token
      expected = ENV['SCHEDULED_NOTIFICATION_API_TOKEN']
      provided = request.headers['X-Api-Token']
      head :unauthorized unless expected.present? && ActiveSupport::SecurityUtils.secure_compare(expected, provided.to_s)
    end
  end
end
